...
class DateCalculator:
    ...
    def is_leap_year(self, year: int) -> bool:
        """ the leap year logic from https://docs.microsoft.com/en-us/office/troubleshoot/excel/determine-a-leap-year
    ...
    def diff(self, start_date: dict, end_date: dict):
        if self.convert_days(start_date) >= self.convert_days(end_date):
            raise ValidationError(f"input end date need to be large than start date")
